...
import { dummyResolver } from '../../composable/dummy.resolver';
...
export const commentResolvers = {
  ...
  Comment: {
    ...
    post: compose(dummyResolver)((comment, args, {db, dataloaders: {postLoader}}: {db: DbConnection, dataloaders: DataLoaders}, info: GraphQLResolveInfo) => {
      return postLoader
    ...
    commentsByPost: compose()((parent, {postId, first=10, offset=0}, {db, requestedFields}: {db: DbConnection, requestedFields: RequestedFields}, info: GraphQLResolveInfo) => {
      ...
      return db.Comment
        .findAll({
          ...
          limit: first,
          offset: offset,
          attributes: requestedFields.getFields(info, {keep:undefined, exclude:undefined})
        })
        .catch(handleError);
    })
  },
